...
from flask import render_template, send_file, Response, redirect, request, url_for, flash
...
print(">>> loaded backend.admin.sold_routes")  # DEBUG při startu – ať víš, že se modul načetl


# ───────────────── LIST: /admin/sold (bez trailing slash) ─────────────────
...
# ──────────────── Faktury – náhled + odeslání e-mailem ───────────────────
...
def sold_invoice_pdf(sold_id: int):
    ...
    return send_file(
        ...
        as_attachment=False,
        download_name=f"faktura_{sold.id}.pdf",
    )


# sjednoceno s tím, co ti dřív ukazoval výpis rout: /admin/sold/<id>/invoice/send
...
def sold_send_invoice(sold_id: int):
    ...
    target = (request.form.get("email") or sold.customer_email or "").strip()
    if not target:
        flash("Chybí e-mail pro odeslání.", "warning")
        return redirect(request.headers.get("Referer") or url_for("admin.sold_list"))

    pdf = build_invoice_pdf_bytes(sold)
    msg = Message(
        subject=f"Faktura #{sold.id} – Náramková Móda",
        recipients=[target],
        body="V příloze posíláme fakturu. Děkujeme za nákup.\n\nNáramková Móda",
    )
    msg.attach(filename=f"faktura_{sold.id}.pdf", content_type="application/pdf", data=pdf)
    try:
        mail.send(msg)
        flash(f"Odesláno na {target}.", "success")
    except Exception as e:
        flash(f"Chyba při odeslání: {e}", "danger")
...
def sold_export_xlsx():
    ...
    try:
        ...
        for r in rows:
            ...
            ws.append([
                date_str, r.name or "", r.quantity or 1, price,
        ...
        return send_file(
            ...
            download_name="sold_export.xlsx",
        ...
        for r in rows:
            date_str = r.sold_at.strftime("%d.%m.%Y %H:%M") if r.sold_at else ""
            w.writerow([
                date_str, r.name or "", r.quantity or 1, str(r.price or ""),
                r.customer_name or "", r.customer_email or "",
