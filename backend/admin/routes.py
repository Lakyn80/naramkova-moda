...
def add_product():
    ...
    POST: odešle data, hlavní obrázek i další media na /api/products/ a vytvoří produkt.
    """
    categories = Category.query.all()

    if request.method == "POST":
        # 1) Sestavíme data z formuláře
        data = {
    ...
    # GET: vykreslíme prázdný formulář
    return render_template("admin/products/add.html", categories=categories)


# ─── Úprava existujícího produktu přes API ───────────────────────────────────
@admin_bp.route("/products/edit/<int:product_id>", methods=["GET", "POST"])
@login_required
def edit_product(product_id):
    """
    GET: zobrazí formulář s předvyplněnými daty produktu.
    POST: odešle PUT na /api/products/<id> s novými daty, hlavním obrázkem i médii.
    """
    ...
    # GET: vykreslíme formulář s daty
    return render_template(
...
# ─── Smazání jednotlivého média (obr./video) ─────────────────────────────────
@admin_bp.route("/admin/media/delete/<int:media_id>", methods=["POST"])
@login_required
def delete_product_media(media_id):
    """
    Smaže soubor z disku i záznam z DB a vrátí zpět na editaci.
    """
